...
@onready var skeleton_node = null
@onready var used = false
@export var scene_name: String = "bag1_scene"
...
func _ready():
	if has_node("Skeleton"):
		...
		print("Skeleton node found and assigned")
	else:
		print("No skeleton node found, skipping")
...
func queue_free_after_unpause() -> void:
	while get_tree().paused:
		...
		skeleton_node.start_jump()
		print("skelly move called")
...
func go_to_the_loot_screen() -> void:
	if warmth.current_sanity <= 25 and skeleton_node != null:
		print("called skelly")
		go_to_the_loot_screen_skeleton()
		return

	if scene_name == "":
		push_error("Scene name not set for this loot!")
		return

	print("Calling dialogue:", scene_name, "from:", resource.resource_path)

	DialogueManager.process_mode = Node.PROCESS_MODE_ALWAYS

	var dialog = DialogueManager.show_example_dialogue_balloon(resource, scene_name)
	if dialog:
		dialog.process_mode = Node.PROCESS_MODE_ALWAYS

	get_tree().paused = true

func go_to_the_loot_screen_skeleton():
	var skeleton_scene_name = scene_name + "skeleton"

	if skeleton_scene_name == "":
		push_error("Scene name not set for skeleton loot!")
		return

	print("Calling dialogue:", skeleton_scene_name, "from:", resource.resource_path)

	DialogueManager.process_mode = Node.PROCESS_MODE_ALWAYS

	var dialog = DialogueManager.show_example_dialogue_balloon(resource, skeleton_scene_name)
	if dialog:
		dialog.process_mode = Node.PROCESS_MODE_ALWAYS
